...
`fd` и `sd` релизы  
`fd` - Framework Dependent релиз. Весит меньше, но ему необходим установленный `.NET Framework` на ПК.  
`sd` - Self Contained релиз. Весит больше. но уже содержит всё необходимое. Установленный `.NET Framework` не нужен.  
